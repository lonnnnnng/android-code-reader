...
| 常用语法 | 高级内容 | 阅读辅助 |
| --- | --- | --- |
| 标题、引用、列表、表格、任务列表、脚注 | YAML Front Matter、代码块高亮、KaTeX 数学公式、Mermaid 流程图、本地图片 | 目录跳转、代码块复制、图片放大、相对附件、跨标签预览缓存、明暗主题、离线渲染 |
...
| Markdown | 源码/预览、Front Matter 折叠、代码高亮、KaTeX、Mermaid、本地图片与附件、图片放大、表格、任务列表、脚注、目录、代码复制 |
...
- Markdown 预览默认禁用原始 HTML 和远程图片联网加载；本地图片与附件只解析当前项目索引内的相对路径，图片、KaTeX 和 Mermaid 单项失败时保留原始内容并提供源码回退，源码/预览滚动同步仍在路线图中。
...
- `Pixel_9` Android 模拟器全量 instrumentation：`49/49` 通过。
- 语法覆盖：`78/78` 通过。
- 覆盖 ZIP 路径安全、Git 克隆/快进更新、项目完整索引、大文件分段搜索、Markdown DOM 与本地资源渲染、设置持久化和在线更新校验。
...
| M2 | Front Matter、本地图片/附件、失败回退、预览缓存已完成；继续源码/预览同步、导出 | 🚧 推进中 |
