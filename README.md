...
| 常用语法 | 高级内容 | 阅读辅助 |
| --- | --- | --- |
| 标题、引用、列表、表格、任务列表、脚注 | YAML Front Matter、代码块高亮、KaTeX 数学公式、Mermaid 流程图、本地图片 | 目录跳转、代码块复制、图片放大、相对附件、跨标签预览缓存、源码/预览位置同步、明暗主题、离线渲染 |
...
| Markdown | 源码/预览、语义块位置同步、Front Matter 折叠、代码高亮、KaTeX、Mermaid、本地图片与附件、图片放大、表格、任务列表、脚注、固定可折叠目录、代码复制 |
...
- `Pixel_9` Android 模拟器全量 instrumentation：`51/51` 通过。
- 语法覆盖：`78/78` 通过。
...
| M2 | Front Matter、本地资源、失败回退、预览缓存、源码/预览同步和固定目录已完成；继续导出与分享 | 🚧 推进中 |
