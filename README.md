...
默认只读，确认进入编辑模式后可对本地文件和可写 SAF 文件做小范围修改；支持撤销/重做、当前/全部替换、正则捕获组、自动缩进、括号与引号补全、2/4/8 Tab 宽度、空格/Tab 缩进、常用行操作、剪贴板操作、多行粘贴缩进优化、自动草稿恢复和安全保存。常用命令集中在紧凑的“编辑操作”面板，只读源码仍可选中当前行和复制内容。灵阅当前不负责编译、运行、调试、完整 LSP 或 IDE 级重构，把资源留给稳定阅读。
...
> `main` 分支已继续推进 M3 编辑能力，但尚未发布新的正式 Release；输入辅助、常用编辑操作、跨进程草稿恢复和安全保存等最新实现需从源码构建 Debug 版本体验。
...
| 轻量编辑 | 撤销/重做、查找替换、自动缩进、括号补全、Tab/空格缩进、选中/删除行、复制/剪切/粘贴、缩进/反缩进、多行粘贴优化、跨进程草稿恢复和安全保存 |
...
- `Pixel_9` Android 模拟器全量 instrumentation：`74/74` 通过。
- 语法覆盖：`78/78` 通过。
- 覆盖 ZIP 路径安全、Git 克隆/快进更新、项目完整索引、大文件分段搜索、Markdown DOM 与本地资源渲染、编辑器输入偏好、常用行与剪贴板操作、跨进程草稿恢复、安全保存、设置持久化和在线更新校验。
...
| M3 | 撤销/重做、查找替换、输入辅助、常用行操作、跨进程草稿恢复和安全保存已实现；继续补不可写来源导出副本 | 🚧 进行中 |
