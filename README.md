...
默认只读，确认进入编辑模式后可对本地文件和可写 SAF 文件做小范围修改；支持撤销/重做、当前/全部替换、正则捕获组、自动草稿恢复和安全保存。灵阅当前不负责编译、运行、调试、完整 LSP 或 IDE 级重构，把资源留给稳定阅读。
...
- `Pixel_9` Android 模拟器全量 instrumentation：`66/66` 通过。
- 语法覆盖：`78/78` 通过。
- 覆盖 ZIP 路径安全、Git 克隆/快进更新、项目完整索引、大文件分段搜索、Markdown DOM 与本地资源渲染、跨进程草稿恢复、安全保存、设置持久化和在线更新校验。
...
| M3 | 撤销/重做、查找替换、跨进程草稿恢复和安全保存已实现；继续补自动缩进与只读来源导出副本 | 🚧 进行中 |
