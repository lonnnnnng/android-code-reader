...
- Markdown 预览默认禁用原始 HTML 和远程图片联网加载；本地图片与附件只解析当前项目索引内的相对路径，源码/预览滚动同步仍在路线图中。
...
- `Pixel_9` Android 模拟器全量 instrumentation：`48/48` 通过。
- 语法覆盖：`78/78` 通过。
...
| M2 | Front Matter、本地图片/附件已完成；继续失败回退、预览缓存、源码/预览同步、导出 | 🚧 推进中 |
