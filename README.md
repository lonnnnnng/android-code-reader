...
- 首页、设置、项目和阅读页统一使用 56dp 紧凑标题栏，同时保留 48dp 图标触控区域；来源入口、设置分类、项目搜索和更新入口复用统一的实体色图标徽标与低层级列表容器，阅读页采用文件名/类型/状态两级标题、编辑器式活动标签和 32dp 可见底板，仍保留 48dp 触控区域。
...
- Pixel_9 模拟器基线已通过 23 项 instrumentation 和 78/78 语法 token 验证；`v0.1.12` 还在 Redmi Note 8 Pro 真机完成 21 项离线测试、2 项联网测试及正式签名包安装启动验证。
