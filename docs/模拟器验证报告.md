...
最终 `lintDebug`、19 项 JVM 单元测试、Debug APK、AndroidTest APK 和 R8 Release APK 均构建成功。Pixel_9 模拟器上的 15 项 instrumentation 测试全部通过，78/78 语法覆盖通过，crash buffer 为空。
...
```text
Starting 15 tests on Pixel_9(AVD) - 15
Finished 15 tests on Pixel_9(AVD) - 15
BUILD SUCCESSFUL
```
...
- 点击分类进入对应二级页；顶部返回与系统返回都会先回到一级设置页，再次返回才退出设置。
