...
最终 `lintDebug`、19 项 JVM 单元测试、Debug APK、AndroidTest APK 和 R8 Release APK 均构建成功。Pixel_9 模拟器上的 17 项 instrumentation 测试全部通过，78/78 语法覆盖通过，crash buffer 为空。

`v0.1.8 (9)` 收紧全局顶部标题栏，统一 Git 克隆弹框的视觉层级与 HTTPS 地址校验，并将在线更新进度固定在更新说明下方、操作按钮上方。应用 ID、GitHub 仓库和 APK 命名协议保持不变；本轮已从 GitHub 公开发布的正式签名 `v0.1.7 (8)` 原地升级到本地正式签名 `v0.1.8 (9)`，冷启动、版本展示和 crash buffer 检查均正常。
...
```text
Starting 17 tests on Pixel_9(AVD) - 15
Finished 17 tests on Pixel_9(AVD) - 15
BUILD SUCCESSFUL
```
...
- 设置页“关于与更新 / 应用更新”显示当前版本 `v0.1.8`。
- 发布前真实请求 `lonnnnnng/android-code-reader` 的公开 GitHub Latest Release `v0.1.7`，确认高版本客户端不会错误提示降级；`v0.1.8` 发布后将再次校验 Latest Release 与在线下载摘要。
...
```text
文件：AndroidCodeReader-v0.1.8.apk
大小：11,070,831 bytes（约 10.6 MiB）
SHA-256：6966cb5f7a3a06ff8fce753b03c99ab3f2c4441d24e63e9d39e15e497727ad8a
```
