...
- 首页右上角提供齿轮设置入口，独立设置页支持实时预览。
- 显示设置覆盖 11-24 sp 字号、跟随主题/柔和灰/护眼绿/暖纸色阅读背景，以及翡翠绿/海洋蓝/琥珀金整体配色；所有选项使用紧凑的下拉选择器。
...
最终 `lintDebug`、17 项 JVM 单元测试、Debug APK、AndroidTest APK 和 R8 Release APK 均构建成功。Pixel_9 模拟器上的 11 项 instrumentation 测试全部通过，78/78 语法覆盖通过，crash buffer 为空。
...
10. `SettingsInstrumentedTest`
   - 从首页进入设置页，修改字号、阅读背景和整体配色；确认固定预览区实时更新，并在 Activity 重建后保持选择。

最终输出：

```text
Starting 11 tests on Pixel_9(AVD) - 15
Finished 11 tests on Pixel_9(AVD) - 15
BUILD SUCCESSFUL
```
...
- 首页右上角齿轮按钮可进入独立设置页，修改选项时预览区立即刷新。
...
```text
文件：AndroidCodeReader-v0.1.3.apk
大小：11,038,007 bytes（约 10.5 MiB）
SHA-256：1090a8107dca79e8182fa9077faf801f9cd5fe3fee28a426e733d4fb92ccbe80
```
