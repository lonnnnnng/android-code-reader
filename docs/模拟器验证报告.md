...
- 首页、设置、项目和阅读页统一使用 60dp 紧凑标题栏，同时保留 48dp 图标触控区域；阅读页使用文件名/类型/状态两级标题、编辑器式活动标签和紧凑上下文工具栏；新增标签会自动滚动到可见区域。
...
`v0.1.9 (10)` 在首页增加稳定可见的“最近打开”菜单和独立管理页，补齐空状态、单项移除及从项目返回列表的导航链路；同时规范化旧版最近记录，并在 SAF 目录重命名后刷新标题。应用 ID、GitHub 仓库和 APK 命名协议保持不变；本轮已从 GitHub 公开发布的正式签名 `v0.1.8 (9)` 原地升级到本地正式签名 `v0.1.9 (10)`，冷启动、版本展示和 crash buffer 检查均正常。
...
- 设置页“关于与更新 / 应用更新”显示当前版本 `v0.1.9`。
- `v0.1.9` 发布后真实请求 `lonnnnnng/android-code-reader` 的公开 GitHub Latest Release，确认正式签名包返回“当前已是最新版本 v0.1.9”，远程 APK 大小、SHA-256 与本地发布产物一致。
...
```text
文件：AndroidCodeReader-v0.1.9.apk
大小：11,070,831 bytes（约 10.6 MiB）
SHA-256：b51e3ea189d3c146506e8ab2998afd3140fa106e345cc4be22576561cd338f05
```
