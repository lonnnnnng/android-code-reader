...
最终 `lintDebug`、20 项 JVM 单元测试、Debug APK、AndroidTest APK 和 R8 Release APK 均构建成功。Pixel_9 模拟器上的 18 项 instrumentation 测试全部通过，78/78 语法覆盖通过，crash buffer 为空。
...
11. `DialogLayoutInstrumentedTest`
   - Git 克隆弹框默认禁用确认按钮，输入合法公开 HTTPS 仓库地址后启用。
   - 更新说明、下载进度和操作按钮按稳定的阅读顺序排列。
12. `AppReleaseParserInstrumentedTest`
...
```text
Starting 18 tests on Pixel_9(AVD) - 15
Finished 18 tests on Pixel_9(AVD) - 15
BUILD SUCCESSFUL
```
...
```text
文件：AndroidCodeReader-v0.1.10.apk
大小：11,070,831 bytes（约 10.6 MiB）
SHA-256：29f8e20dc60dc4c7244b7c913bb70db955df7171922a3992e8d870e4f3bcd84b
```
